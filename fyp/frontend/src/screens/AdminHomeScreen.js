...
const AdminHomeScreen = () => {
  ...
  const { state, fetchWorkers, getReviews, blockWorker } =
  ...
  const handleRecommendationPress = async (worker) => {
    // Display options like "Block" using a modal or other UI element
    // When "Block" option is selected, call blockWorker with worker ID
    // console.log(worker._id);
    const confirmBlock = await Alert.alert(
      "Confirm Block",
      `Are you sure you want to block ${worker.username}?`,
      [
        {
          text: "Cancel",
          style: "cancel",
        ...
        {
          text: "Block",
          onPress: () => blockWorker({ id: worker._id }),
        },
      ]
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomBar}>
        ...
        <TouchableOpacity
          style={styles.bottomBarItem}
          onPress={() => navigation.navigate("UnBlockWorkerScreen")}
        >
